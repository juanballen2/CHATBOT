...
async function loadOmniChats() {
    try {
        ...
        omniChats = allChats.filter(chat => {
            ...
            // o si el 'source' en la tabla leads es uno de estos.
...
function handleKeyboardShortcuts(e) {
    ...
    // 2. Respuesta rápida con '/' (Opcional, si tienes respuestas predefinidas)
    // if (e.key === '/' && msgInput.value.length === 0) {
    //     mostrarMenuRespuestasRapidas();
    // }
    
    // 3. Abrir/Cerrar panel CRM con Alt + 1
    if (e.altKey && e.key === '1') {
        e.preventDefault();
        toggleCRM();
    }
