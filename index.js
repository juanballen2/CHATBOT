...
// Configuración de almacenamiento en memoria para subida de archivos
...
const SESSION_SECRET = "icc-valentina-v12-5-full"; 

// Aumentamos el límite para permitir subida de archivos grandes
app.use(express.json({ limit: '50mb' }));
app.use(express.urlencoded({ limit: '50mb', extended: true }));
app.use(cors());

// Middleware de seguridad para proteger archivos de datos
...
// Función auxiliar para normalizar teléfonos (Asume Colombia 57 si falta)
function normalizePhone(phone) {
    let p = phone.replace(/\D/g, ''); // Quitar no-números
    if (p.startsWith('57')) return p;
    if (p.length === 10) return '57' + p; // Si es celular 3xx sin 57, agregarlo
    return p; // Retorno tal cual si no cumple lógica básica
}
...
(async () => {
    ...
    // Migraciones seguras para evitar errores si ya existen las tablas
    ...
    console.log("🚀 VALENTINA v12.5 - BUSINESS SUITE ONLINE (UNCUT VERSION)");
})();
...
async function enviarWhatsApp(destinatario, contenido, tipo = "text") {
    try {
        ...
        if (tipo === "text") { 
            ...
            // Estructura específica para plantillas (usamos hello_world por defecto para iniciar)
            payload.template = { 
                name: "hello_world", 
                language: { code: "en_US" } 
            };
...
// Función para subir archivos a Meta (Audio corregido a audio/ogg)
...
// Proxy para visualizar archivos (imágenes/videos) en el dashboard
...
async function procesarConValentina(message, sessionId, mediaDesc = "") {
    ...
    ROL: Valentina, asistente de ICC.
    [DATOS EMPRESA]
    Nombre: ${bizProfile.name || 'ICC'}
    ...
    TU META: Filtrar, tomar datos (Nombre, Ciudad, Maquina/Repuesto) y transferir. NO inventar precios.
    
    [OUTPUT JSON]
    Si detectas datos nuevos:
    \`\`\`json
    ...
    try {
        ...
        return textoVisible;
    } catch (err) { return "Estoy validando un dato, un momento por favor."; }
}
...
async function gestionarLead(phone, info) {
    ...
    if (!nombreFinal || nombreFinal === "null") nombreFinal = leadExistente?.nombre || meta?.contactName || "Cliente WhatsApp";

    let esMisma = false;
    if (leadExistente) {
        if ((new Date() - new Date(leadExistente.fecha))/(1000*60*60) < 24) esMisma = true;
    }

    const datos = {
        nombre: nombreFinal,
        interes: info.interes || (esMisma ? leadExistente.interes : "General"),
        ciudad: info.ciudad || (esMisma ? leadExistente.ciudad : "No indicada"),
        correo: info.correo || (esMisma ? leadExistente.correo : "No indicado"),
        etiqueta: info.etiqueta || "Lead"
    };

    if (esMisma) {
        await db.run(`UPDATE leads SET nombre=?, interes=?, etiqueta=?, ciudad=?, correo=?, fecha=? WHERE id = ?`,
...
app.post('/api/chat/send', proteger, async (req, res) => {
    ...
    } else res.status(500).json({ error: "Error enviando. Posible ventana de 24h cerrada." });
});

// NUEVO: Enviar Plantilla de Inicio (Hello World) para romper la ventana de 24h
app.post('/api/chat/send-template', proteger, async (req, res) => {
    if(await enviarWhatsApp(req.body.phone, null, "template")) {
        await db.run("INSERT INTO history (phone, role, text, time) VALUES (?, ?, ?, ?)", [req.body.phone, 'manual', "[PLANTILLA INICIO ENVIADA]", new Date().toISOString()]);
        res.json({ success: true });
    } else res.status(500).json({ error: "Error enviando plantilla" });
});
...
app.post('/api/leads/update', proteger, async (req, res) => {
    ...
    if(!allowed.includes(field)) return res.status(400).send("Campo invalido");
...
app.post('/api/tags/add', proteger, async (req, res) => {
    try { await db.run("INSERT INTO global_tags (name, color) VALUES (?, ?)", [req.body.name, req.body.color]); res.json({success:true}); } catch(e){res.status(400).json({error:"Ya existe"});}
});
...
// GESTIÓN SHORTCUTS (Respuestas rápidas)
app.post('/api/shortcuts/add', proteger, async (req, res) => {
    try { await db.run("INSERT INTO shortcuts (keyword, text) VALUES (?, ?)", [req.body.keyword, req.body.text]); res.json({success:true}); } catch(e){res.status(400).json({error:"Ya existe"});}
});
...
app.post('/api/config/biz/save', proteger, async (req, res) => {
    ...
    await setCfg('website_data', req.body.website_data);
    res.json({success:true});
});

// SUBIDA DE FOTO DE PERFIL DEL CLIENTE (Workaround)
...
// AGREGAR CONTACTO (Normalizando teléfono)
app.post('/api/contacts/add', proteger, async (req, res) => { 
    const cleanPhone = normalizePhone(req.body.phone);
    await db.run("INSERT INTO metadata (phone, contactName, addedManual) VALUES (?, ?, 1) ON CONFLICT(phone) DO UPDATE SET contactName=excluded.contactName", [cleanPhone, req.body.name]); 
...
app.post('/api/chat/upload-send', proteger, upload.single('file'), async (req, res) => {
    try {
        ...
        if(mediaId) {
            ...
            res.json({success: true});
        } else res.status(500).json({error: "Meta Upload Failed"});
    } catch(e) { res.status(500).json({error: e.message}); }
});

// CSV Inventory
app.post('/api/knowledge/csv', proteger, upload.single('file'), async (req, res) => { try { const n = parse(req.file.buffer.toString('utf-8'), { columns: true }); for (const r of n) await db.run("INSERT OR IGNORE INTO inventory (searchable, raw_data) VALUES (?, ?)", [Object.values(r).join(" "), JSON.stringify(r)]); await refreshKnowledge(); res.json({ success: true }); } catch(e){res.status(500).send("Error CSV");} });
...
app.post('/api/test-ai', proteger, async (req, res) => { try { const r = await axios.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${API_KEY}`, { contents: [{ parts: [{ text: `ERES VALENTINA. USER: ${req.body.message}` }] }] }); res.json({ response: r.data.candidates[0].content.parts[0].text }); } catch(e) { res.status(500).json({ error: e.message }); } });
...
app.post('/webhook', async (req, res) => {
    ...
    try {
        ...
        if (value?.contacts?.[0]) {
            ...
            await db.run("INSERT INTO metadata (phone, contactName) VALUES (?, ?) ON CONFLICT(phone) DO UPDATE SET contactName=contactName WHERE addedManual=0", [phone, name]);
        }
        ...
        if(msg) {
            ...
            else if (msg.type === "video") { userMsg = `[MEDIA:VIDEO:${msg.video.id}]`; mediaDesc = "🎥 Video"; }
            ...
            const inputIA = mediaDesc ? mediaDesc : userMsg;
...
app.listen(process.env.PORT || 10000, () => console.log("🚀 VALENTINA v12.5 FULL ONLINE"));
